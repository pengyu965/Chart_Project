        ...
        for i in range(x):
            for j in range(y):
                ...
                if _class == 2 and np.random.rand()>0.8:
                    cv2.arrowedLine(new_arr, (i,j), (int(i+arr[i,j,6]*10), int(j+arr[i,j,7]*10)), (0,0,255),1)
                if _class == 4 and np.random.rand()>0.8:
                    cv2.arrowedLine(new_arr, (i,j), (int(i+arr[i,j,6]*10), int(j+arr[i,j,7]*10)), (0,255,0),1)
