    ...
    if regression_vis == True:
        for i in range(x):
            for j in range(y):
                ...
                if _class == 2:
                    cv2.arrowedLine(new_arr, (i,j), tuple(int(i+arr[i,j,6]*50), int(j+arr[i,j,7]*50)), (0,0,255),1)
                if _class == 4:
                    cv2.arrowedLine(new_arr, (i,j), tuple(int(i+arr[i,j,6]*50), int(j+arr[i,j,7]*50)), (0,255,0),1)
