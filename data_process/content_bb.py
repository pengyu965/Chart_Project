    ...
    try: 
        ...
        for axis in task4["output"]["axes"]:
            for tick in task4["output"]["axes"][axis]:
                ...
                # color_id += 1
        # cv2.imwrite("../data/SUMIT/axes_bb/"+image, img)
        # cv2.imshow("Image Example",img)
        # cv2.waitKey(0)
    except:
        shutil.move(os.path.join(data_path,image), "./data/SUMIT/badsamples/images/")
        shutil.move(os.path.join(json_gt_path, image_name+".json"), "./data/SUMIT/badsamples/json_gt/")
