...
def ticks_gt_gen(file_name):
    ...
    h,w,c = img.shape
    rs_img = cv2.resize(img, (512,512), interpolation=cv2.INTER_AREA)
    ...
    for text_bb in needed_dic["input"]["task2_output"]["text_blocks"]:
        for item in text_bb["bb"]:
            if item == "height" or item =="y0":
                text_bb["bb"][item] = round(text_bb["bb"][item] * 512/h)
            if item == "width" or item =="x0":
                text_bb["bb"][item] = round(text_bb["bb"][item] * 512/w)
    ...
    needed_dic["input"]["task4_output"]["_plot_bb"]["height"] = round(needed_dic["input"]["task4_output"]["_plot_bb"]["height"]*512/h)
    needed_dic["input"]["task4_output"]["_plot_bb"]["y0"] = round(needed_dic["input"]["task4_output"]["_plot_bb"]["y0"]*512/h)
    needed_dic["input"]["task4_output"]["_plot_bb"]["x0"] = round(needed_dic["input"]["task4_output"]["_plot_bb"]["x0"]*512/w)
    needed_dic["input"]["task4_output"]["_plot_bb"]["width"] = round(needed_dic["input"]["task4_output"]["_plot_bb"]["width"]*512/w)
    for axis in needed_dic["input"]["task4_output"]["axes"]:
        for sub_dic in needed_dic["input"]["task4_output"]["axes"][axis]:
            sub_dic["tick_pt"]["x"] = round(sub_dic["tick_pt"]["x"]*512/w)
            sub_dic["tick_pt"]["y"] = round(sub_dic["tick_pt"]["y"]*512/h)
